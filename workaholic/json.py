...
def json_view(required_parameters=None):
    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            missing_parameters = set(required_parameters) - set(json_parameters.keys())
            if missing_parameters:
                return HttpResponseBadRequest(json.dumps(dict(
                    success=False,
                    error=_('Missing parameters: %s') % ', '.join(missing_parameters)
                )))
            parameters = [json_parameters[parameter] for parameter in required_parameters]
            ...
            # may be returned.
            resp = view_func(request, *parameters, *args, **kwargs)
